    ...
    <script>
        ...
        function submitBtn() {
            $("#submit").click(function(e) {
                if (status == "accepted" || status == "waiting" || status == "preparing") {
                    Swal.fire({
                        title: 'Information',
                        text: 'Order status not prepared yet by admin, please wait for admin to prepare your order',
                        icon: 'info',
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('order.preparasiConfirm') }}",
                        data: form,
                        dataType: "json",
                        contentType: false,
                        cache: false,
                        ...
                        success: function(result) {
                            ...
                            setTimeout(function() {
                                $(location).attr('href', '{{ route('user.labtest') }}');
                            }, 3000);
                        },
